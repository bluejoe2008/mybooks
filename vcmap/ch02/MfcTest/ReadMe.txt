...
这些文件包含 CMfcTestDoc 类。编辑这些文件可以添加特殊文档数据并可实现文件保存和加载(通过 CMfcTestDoc::Serialize)。

MfcTestView.h，MfcTestView.cpp - 文档的视图
这些文件包含 CMfcTestView 类。
CMfcTestView 对象用于查看 CMfcTestDoc 对象。

res\MfcTestDoc.ico
这是图标文件，它用作 CMfcTestDoc 类的 MDI 子窗口的图标。此图标包括在主要资源文件 MfcTest.rc 中。
