...
//线程函数
UINT WorkerThreadWithUI(LPVOID pParam)
{
	CWorkerThreadTestDlg * pDialog = (CWorkerThreadTestDlg *)pParam;
	CProgressCtrl * pProgressCtrl = &pDialog->m_ProgressCtrl;
	...
	for(int n = 0; n < 100; n++)
	{
		...
		s.Format(_T("%d%%"), n);
		//显示进度百分比
		pDialog->m_ProgressText.SetWindowText(s);
...
void CWorkerThreadTestDlg::OnBnClickedButton1()
{
	...
	//传递当前窗口指针至线程函数
	AfxBeginThread(WorkerThreadWithUI, (LPVOID)this);
}
