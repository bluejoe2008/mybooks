...
struct MyParameter
{
	...
	CWorkerThreadTestDlg * pDialog; //对话框
};

UINT WorkerThreadA2(LPVOID pParam)
{
	MyParameter * pp = (MyParameter *)pParam;

	while(pp->isRunning)
	{
		pp->pDialog->m_ProgressCtrl.SetPos(pp->count);
		pp->pDialog->m_ProgressText.SetWindowText(pp->message);

		//休息100毫秒，否则文字会闪烁
		Sleep(100);
	}

	pp->pDialog->MessageBox(_T("WorkerThreadA线程结束！"));
	delete pp;
	return 0;
}

UINT WorkerThreadB2(LPVOID pParam)
{
	MyParameter * pp = (MyParameter *)pParam;
	...
	for(int i = 0; i <= 100; i++)
	{
		//设定进度
		pp->count++;
		pp->message.Format(_T("%d%%"), i);

		//休息100毫秒
		Sleep(100);
	}

	pp->pDialog->MessageBox(_T("WorkerThreadB线程结束！"));
	pp->isRunning = false;
...
void CWorkerThreadTestDlg::OnBnClickedButton1()
{
	...
	m_ProgressCtrl.SetRange(0, 100);
	MyParameter * pParameter = new MyParameter();
	pParameter->pDialog = this;
	pParameter->count = 0;
	pParameter->isRunning = true;

	AfxBeginThread(WorkerThreadA2, (LPVOID)pParameter);
	AfxBeginThread(WorkerThreadB2, (LPVOID)pParameter);
}
