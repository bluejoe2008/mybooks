...
void CFlashPlayerDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_EDIT1, m_MoviePath);
}
...
BEGIN_MESSAGE_MAP(CFlashPlayerDlg, CDialog)
	...
	ON_WM_QUERYDRAGICON()
	//}}AFX_MSG_MAP
	ON_BN_CLICKED(IDC_BUTTON2, &CFlashPlayerDlg::OnBnClickedButton2)
END_MESSAGE_MAP()
...
void CFlashPlayerDlg::OnBnClickedButton2()
{
	CFileDialog FileDialog(true, _T(".swf"), _T(""), OFN_EXPLORER,
		_T("Flash动画文件(*.swf)|*.swf|所有文件(*.*)|*.*||"), this);

	if(FileDialog.DoModal() == IDOK)
	{
		m_MoviePath = FileDialog.GetPathName();
		UpdateData(false);

		m_FlashCtrl.put_Movie(m_MoviePath);
	}
}
